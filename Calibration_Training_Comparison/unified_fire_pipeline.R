...
# reads OUTPUT_FILE.
#
#To execute: nohup Rscript unified_fire_pipeline.R > pipeline_log_$(date +%Y%m%d_%H%M).txt 2>&1 &
# =============================================================================
...
CLIMATE_SOURCE <- "trace"

## ---- Output ---------------------------------------------------------------
OUTPUT_FILE <- "predictions_calibration_posterior_trace_no_outliers_cap.csv"
...
base_path <- "/users/c/u/cuden/raid/cuden/Paleo_Lightning_Fire_Uden_2026"  # <-- CONFIRM/EDIT
...
area_lookup <- area_lookup %>%
  ...
  mutate(lat = round(lat, 4), lon = round(lon, 4))   # guard against the same
# floating-point join-key
# mismatch fixed elsewhere
# in this script
...
if (PERIOD == "calibration") {
  ...
  # Calibration window is 1949-1989 (1949 as a lead-in year for lagged
  # predictors, e.g. precip_5y/tair_5y). No upper bound was previously set,
  # which silently let an incomplete/out-of-range year (1990) past the end
  # of the source data through -> extreme standardized predictor values ->
  # theta_pred overflow -> NA n_strikes for every gridcell that year.
  calib_year_min <- 1949
  calib_year_max <- 1989
  
  if (CLIMATE_SOURCE == "era5") {
    summer_df <- read_parquet(climate_summer_path) %>% filter(year >= calib_year_min & year <= calib_year_max)
    ...
    month_df <- read_parquet(climate_month_path) %>% filter(month >= 6 & year >= calib_year_min & year <= calib_year_max)
    ...
    month_df  <- read_parquet(climate_month_path) %>%
      ...
      filter(month >= 6 & year >= calib_year_min & year <= calib_year_max)
    month_df <- month_df %>% mutate(lat = round(lat, 4), lon = round(lon, 4))
  }
...
# holding all N_DRAWS results in memory -- for large datasets, accumulating
# everything until the end can exhaust available RAM and cause severe
# slowdown from swapping, with no visible progress. This also lets you
# check real progress at any time with, from a terminal: wc -l <OUTPUT_FILE>
