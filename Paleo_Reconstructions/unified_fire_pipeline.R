...
## ---- Output ---------------------------------------------------------------
OUTPUT_FILE <- "predictions_paleo_posterior_no_outliers_cap.csv"
...
area_lookup <- area_lookup %>%
  ...
  mutate(lat = round(lat, 4), lon = round(lon, 4))   # guard against floating-point
# join-key mismatches between
...
if (PERIOD == "calibration") {
  ...
  # predictors, e.g. precip_5y/tair_5y). No upper bound previously let an
  # incomplete/out-of-range year (1990) past the end of the source data
  # through -> extreme standardized predictor values -> theta_pred overflow
  # -> NA n_strikes for every gridcell that year.
  calib_year_min <- 1949
  calib_year_max <- 1989
